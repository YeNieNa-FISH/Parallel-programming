...
void init_SSE(){
    m_data = (float**) _mm_malloc(n * sizeof(float*), 16);
    for (int i = 0; i < n; i++) {
        m_data[i] = (float*) _mm_malloc(n * sizeof(float), 16);
    }
    srand((int)time(0));
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < n; j++) {
...
int main() {
	...
	for (int i = 0; i < 9; i++) {
        ...
        cout << n << ": ";
		for(int j =0;j<10;j++){
            //m_data = (float**) _mm_malloc(n * sizeof(float*), 16); //SSE
            ...
            //cout << n << "用时\t\t" << duration_cast<nanoseconds>(t2 - t1).count() << '\n';
